...
struct Colors {
  ...
  static var panelBackground = NSColor(hex: "1e1e2d")!.withAlphaComponent(1)  // Nord 1 (Polar Night)
  static var cardBackground = NSColor(hex: "1e1e2d")!.withAlphaComponent(0.95)  // Nord 2 (Polar Night)
  static var graphBackground = NSColor(hex: "1e1e2d")!  // Nord 3 (Polar Night)

  // Accents
  static var accent = NSColor(hex: "92b3f5")!  // Nord 4 (Snow Storm)
  static var blue = NSColor(hex: "92b3f5")!  // Nord 5 (Snow Storm)
  static var green = NSColor(hex: "b3e1a7")!  // Nord 6 (Frost)
  static var yellow = NSColor(hex: "f5e3b5")!  // Nord 7 (Frost)
  static var orange = NSColor(hex: "efb68e")!  // Nord 8 (Frost)
  static var red = NSColor(hex: "e0a3ac")!  // Nord 9 (Frost)
...
class AppDelegate: NSObject, NSApplicationDelegate {
  ...
  func applicationDidFinishLaunching(_ notification: Notification) {
    ...
    let gapSize =
      Int(
        String(data: gapsData, encoding: .utf8)?.trimmingCharacters(in: .whitespacesAndNewlines)
          ?? "0") ?? 0
      ...
      VStack(spacing: 10) {
        ...
        LazyVGrid(
          ...
          CircularProgressView(
            ...
            icon: batteryIcon(
              level: statsController.batteryLevel, isCharging: statsController.isCharging),
...
class StatsController: ObservableObject {
  ...
  private func updateMemoryUsage() {
    ...
    if result == KERN_SUCCESS {
      ...
      let total =
        Double(stats.active_count + stats.inactive_count + stats.wire_count + stats.free_count)
        * Double(pageSize)
...
class WeatherController: ObservableObject {
  ...
  private func updateWeather() {
    ...
    let urlString =
      "https://api.openweathermap.org/data/2.5/forecast?q=vijayawada&appid=14604ad4b732993b6d3808a8711202d2&units=metric"
    ...
    VStack(spacing: 12) {
      ...
      HStack(spacing: 8) {
        ForEach(weekDays, id: \.self) { day in
          Text(day)
            .font(.system(size: 12))
            .foregroundColor(day == "S" ? Color(Colors.accent) : Color.white)
            .frame(width: 20)
        }
      }
