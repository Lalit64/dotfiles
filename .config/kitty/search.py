...
class Search(Handler):
    ...
    def update_prompt(self) -> None:
        self.prompt = "\033[93m(Reg)❯ " if self.mode == "regex" else "\033[94m❯ "
    ...
    def on_key(self, key_event: KeyEventType) -> None:
        ...
        if key_event.matches("cmd+u"):
            self.line_edit.clear()
            self.refresh()
        elif key_event.matches("cmd+a"):
            self.line_edit.home()
            self.refresh()
        elif key_event.matches("cmd+e"):
            self.line_edit.end()
            self.refresh()
        elif key_event.matches("cmd+backspace") or key_event.matches("ctrl+w"):
